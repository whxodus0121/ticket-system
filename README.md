...
- ### ⚫ v7.0: 분산 대기열(Virtual Queue) 고도화 및 시스템 안정성 최종 완성
- **주요 내용**: 고부하 상황에서 서버 리소스를 보호하기 위한 가상 대기열 도입 및 장애 복구 로직 구현
  - Virtual Waiting Queue: Redis Sorted Set을 활용하여 WAITING 유저에게는 실시간 순번(Rank)을 제공하고, ACTIVE 유저만 실제 예매 로직을 수행하도록 Throttling 구현

   ![대기열에 있던 유저가 Active Set으로 들어가는 과정](./images/Promoter_log.jpg)
  
- **결과**: 
  - **트래픽 제어 능력 확보**: 수만 명의 동시 접속 상황에서도 서버가 감당 가능한 수준의 부하만 유지하며 안정적으로 가동됨을 확인
  - **데이터 무결성 및 내결함성 완성**: 비동기 대기열 통과 후 발생할 수 있는 소수의 실패 건수까지 DLQ로 완벽하게 격리 및 복구 가능한 상태로 프로젝트 종료
